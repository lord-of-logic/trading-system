...
class BuyOrderTypeProcessor(
    ...
    override fun executeTradeByStockId(stockId: Long, order: Order) {
        ...
        //ToDo: Add concurrency to execute trades in parallel for each sell order
        while (order.remainingQuantity!! > 0 && eligibleSellOrderQuantity > 0 && eligibleSellOrders.isNotEmpty()) {
            ...
            val executedTrade = tradeService.executeTrade(stock, order, sellOrder, TradeType.BUY, ordersToUpdateAsCompleted)
            eligibleSellOrderQuantity -= executedTrade.quantity!!
